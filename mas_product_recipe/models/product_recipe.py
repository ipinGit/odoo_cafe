...
class ProductRecipe(models.Model):
    ...
    @api.one
    def compute_raw_material(self, recipe, cols={}, rec_qty=1):
        for ing in recipe.ingredient_ids:
            rec = self.search([('product_id', '=', ing.product_id.id)])
            if rec:
                return compute_raw_material(rec, ing.qty)
            else:
                q = ing.get_converted_uom()*rec_qty
                if ing.product_id.id in cols:
                    cols.update({ing.product_id.id: q + cols.get(product_id.id)})
                else:
                    cols.update(ing.product_id.id: q)
                return cols
